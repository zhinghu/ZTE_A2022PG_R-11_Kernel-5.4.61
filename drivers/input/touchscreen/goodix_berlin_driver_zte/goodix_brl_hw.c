...
int zte_finger_onekey_switch(struct goodix_ts_core *cd, int finger_type, int onekey_type)
{
	struct goodix_ts_cmd cmd;

	cmd.cmd = 0x14;
	cmd.len = 0x6;
	cmd.data[0] = finger_type;
...
int zte_enter_play_game(struct goodix_ts_core *cd)
{
	struct goodix_ts_cmd cmd;
...
static struct goodix_ts_hw_ops brl_hw_ops = {
	...
	.set_finger_onekey_switch = zte_finger_onekey_switch,
#if defined(ZTE_FINGERAREA_A30PRO)
