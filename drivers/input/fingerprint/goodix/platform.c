...
extern unsigned int goodix_fp_id;
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	/* if fp_id is 1, set gpio fp_id output */
	gf_dev->id_gpio = of_get_named_gpio(np, "fp-gpio-id", 0);
	if (gf_dev->id_gpio < 0) {
		pr_err("falied to get id gpio!\n");
	} else {
		pr_info("gf_dev->id_gpio, gpio = %d\n", gf_dev->id_gpio);
		rc = devm_gpio_request(dev, gf_dev->id_gpio, "goodix_id");
		if (rc) {
			pr_err("%s: failed to request id gpio, rc = %d\n", __func__, rc);
		} else {
#ifdef ZTE_FINGERPRINT_FP_ID
			gpio_direction_output(gf_dev->id_gpio, 0);
			pr_info("%s: set fp_id output\n", __func__);
#else
			pr_info("%s: don't change fp_id state\n", __func__);
#endif
		}
	}
	...
	pr_info("gf_dev->reset_gpio, state:0, gpio = %d, rc = %d\n", gf_dev->reset_gpio, rc);
	...
	pr_info("gf_dev->reset_gpio, state:1, gpio = %d, rc = %d\n", gf_dev->reset_gpio, rc);
