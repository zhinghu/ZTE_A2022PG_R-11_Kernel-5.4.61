...
static ssize_t goodix_reset_write(struct file *file, const char __user *buffer,
			size_t count, loff_t *f_pos)
{
	struct gf_dev *gf_dev = &gf;

	pr_info("goodix_reset_write");
...
static const struct file_operations goodix_reset_proc_fops = {
	.owner		= THIS_MODULE,
	.write		= goodix_reset_write,
};

unsigned int goodix_fp_id = 0;
static ssize_t goodix_id_write(struct file *file, const char __user *buffer,
			size_t count, loff_t *f_pos)
{
	char *temp = NULL;
	...
	pr_info("%s: id=%d\n", __func__, goodix_fp_id);

	return count;
}

static int goodix_id_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", goodix_fp_id);
	pr_info("%s: id=%d\n", __func__, goodix_fp_id);
...
static int goodix_id_open(struct inode *inode, struct file *file)
{
	return single_open(file, goodix_id_show, NULL);
}

static const struct file_operations goodix_id_proc_fops = {
	.owner		= THIS_MODULE,
	.open		= goodix_id_open,
	.write		= goodix_id_write,
};
...
#endif
{
	...
	/* for reset test */
	proc_create("driver/fp_reset", 0664, NULL, &goodix_reset_proc_fops);
	proc_create("driver/fp_id", 0664, NULL, &goodix_id_proc_fops);
