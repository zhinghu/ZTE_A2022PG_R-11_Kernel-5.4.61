...
enum blink_mode {
	AW_CONST_OFF = 0, /*0 darkness is coming.*/
					...
					const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%u %u %u %u %u %u %u", &breath_param.breath_mode,
											  &breath_param.fade_time,
											  &breath_param.fullon_time,
											  &breath_param.fulloff_time,
											  &breath_param.min_grade,
											  &breath_param.max_grade,
											  &breath_param.red_green_blue);
	if (ret == 0) {
		pr_err("breath_feature_store sscanf failure ret=0\n");
		return 0;
	}

	pr_debug("breath_feature_store sscanf success ret=%d\n", ret);

	if (breath_param.red_green_blue == 3)
