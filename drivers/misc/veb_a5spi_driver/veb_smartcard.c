...
long veb_smartcard_ioctl(veb_private_data_t *priv, unsigned int cmd, unsigned long arg)
{
	...
	if (data.type == VEB_DATA_MMAP) {
		...
		buf = priv->mmap_addr;
	} else if ((data.type == VEB_DATA_COPY) && (data.outbuf != NULL) && (data.inbuf != NULL)) {
	...
	ret = veb_smartcard_cmd(priv->spi, data.cmd, buf, data.inlen, buf, &uOutLen);
...
long veb_smartcard_compat_ioctl(veb_private_data_t *priv, unsigned int cmd, unsigned long arg)
{
	veb_smartcard_data_t data;
	long ret = VEB_OK;
	char *buf = NULL;
