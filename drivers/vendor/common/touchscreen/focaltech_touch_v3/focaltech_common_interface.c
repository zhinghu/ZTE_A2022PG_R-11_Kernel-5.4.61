...
#ifdef FTS_MODULE4_ID
	{FTS_MODULE4_ID, FTS_MODULE4_LCD_NAME },
#endif
	{VENDOR_END, "Unknown"},
};
...
static int tpd_test_cmd_show(struct tpd_classdev_t *cdev, char *buf)
{
	...
	FTS_INFO("tpd test result:%d && rawdata node failed count:%d.\n", fts_tptest_result, fts_test_failed_count);
...
#ifdef FTS_HEADSET_DETECT
static int fts_headset_state_show(struct tpd_classdev_t *cdev)
{
	struct fts_ts_data *ts_data = fts_data;
...
static int fts_set_headset_state(struct tpd_classdev_t *cdev, int enable)
{
	struct fts_ts_data *ts_data = fts_data;
...
static int fts_set_display_rotation(struct tpd_classdev_t *cdev, int mrotation)
{
	int ret = 0;
	struct fts_ts_data *ts_data = fts_data;
...
static int fts_charger_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if ((strcmp(psy->desc->name, "usb") == 0)
		...
		if (delayed_work_pending(&ts_data->charger_work)) {
			return NOTIFY_DONE;
		}
...
int tpd_register_fw_class(struct fts_ts_data *data)
{
	...
	tpd_fw_cdev.set_display_rotation = fts_set_display_rotation;
#ifdef FTS_HEADSET_DETECT
	tpd_fw_cdev.headset_state_show = fts_headset_state_show;
	tpd_fw_cdev.set_headset_state = fts_set_headset_state;
#endif
#ifdef FTS_ESDCHECK_REGISTER
