...
const struct ts_firmware *syna_tp_requeset_firmware(char *file_name)
{
	...
	if (IS_ERR(file)) {
		pr_err("open %s file fail, try open /vendor/firmware/", file_path);
	...
	if (firmware == NULL) {
		pr_err("Request from file alloc struct firmware failed");
		goto err_close_file;
	}
	file_len = file_inode(file)->i_size;
	firmware->size = (int)file_len;
	pr_notice("open %s file ,firmware->size:%d", file_path, firmware->size);
	firmware->data = vmalloc(firmware->size);
	if (firmware->data == NULL) {
		pr_err("alloc firmware data failed");
	...
	if (ret < 0) {
		pr_err("Request from fs read whole file failed %d", ret);
...
static int fwu_start_reflash(void)
{
	...
	if (!fwu->force_update) {
		...
		firmware = syna_tp_requeset_firmware(syna_file_name);
		if (firmware == NULL) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Firmware image %s not available\n",
				__func__, fwu->image_name);
