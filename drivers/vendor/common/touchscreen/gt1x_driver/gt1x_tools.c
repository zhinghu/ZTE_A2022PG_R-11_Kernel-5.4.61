...
{
	snprintf(procname, PRO_NAME_LENGTH, "gmnode"); /* modify for moto */
}
...
static ssize_t gt1x_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	if (cmd_head.wr == 1) {
		...
		memcpy(IC_TYPES, cmd_head.data, sizeof(IC_TYPES));
		/*ret = copy_from_user(IC_TYPES, cmd_head.data, cmd_head.data_len);
		if (ret) {
			GTP_ERROR("[WRITE]copy_from_user failed.");
			return -EINVAL;
		}*/
		...
		/* memcpy(IC_TYPES, cmd_head.data, cmd_head.data_len); */
		return cmd_head.data_len + CMD_HEAD_LENGTH;
...
static ssize_t gt1x_tool_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!buffer) {
		GTP_ERROR("buffer cannot be null!");
		return -EINVAL;
	}
	if (*ppos) {
	...
	if (cmd_head.wr % 2) {
		...
		*ppos += 4;
		...
		ret = copy_to_user(buffer, GTP_DRIVER_VERSION, sizeof(GTP_DRIVER_VERSION));
		if (ret) {
			GTP_ERROR("[READ]copy_to_user failed!");
			return -EINVAL;
		}
