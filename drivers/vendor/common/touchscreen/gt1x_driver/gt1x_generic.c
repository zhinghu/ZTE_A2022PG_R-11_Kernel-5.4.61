...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	ptr = kzalloc(TP_INFO_LENGTH, GFP_KERNEL);
	if (!ptr)
		return -ENOMEM;

	len += snprintf(ptr + len, TP_INFO_LENGTH - len,
		" == == GT1X default config setting in driver == ==\n");

	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		len += snprintf(ptr + len, TP_INFO_LENGTH - len, "0x%02X, ", gt1x_config[i]);
		if (i % 10 == 9)
			len += snprintf(ptr + len, TP_INFO_LENGTH - len, "\n");
	}
	...
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		len += snprintf(ptr + len, TP_INFO_LENGTH - len, "0x%02X, ", temp_data[i]);
		if (i % 10 == 9)
			len += snprintf(ptr + len, TP_INFO_LENGTH - len, "\n");
	}
	...
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "\n");
	/* Touch PID & VID */
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, " == == GT1X Version Info == ==\n");
	...
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "ProductID: GT%c%c%c%c\n",
		temp_data[0], temp_data[1], temp_data[2], temp_data[3]);
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "PatchID: %02X%02X\n",
		temp_data[4], temp_data[5]);
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "MaskID: %02X%02X\n",
		temp_data[7], temp_data[8]);
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "SensorID: %02X\n",
		temp_data[10] & 0x0F);
...
s32 gt1x_send_cfg(u8 *config, int cfg_len)
{
	...
	if (update_info.status) {
		GTP_INFO("Ignore cfg during fw update.");
		return -EINVAL;
	}

	mutex_lock(&mutex_cfg);
	GTP_INFO("Driver send config, length:%d", cfg_len);
	...
	checksum = 0 - checksum;
	GTP_INFO("Config checksum: 0x%04X", checksum);
...
static int gt1x_ps_init(void)
{
	...
	/* create sysfs files */
	{
		...
		for (i = 0; i < ARRAY_SIZE(ps_attrs); i++) {
