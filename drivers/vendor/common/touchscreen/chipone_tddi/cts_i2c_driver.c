...
static int cts_i2c_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret =  get_tp_chip_id();
	if (ret == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) &&
			(tpd_fw_cdev.tp_chip_id != TS_CHIP_CHIPONE)) {
			cts_info("this tp is not used,return.\n");
			return -EPERM;
		}
	}
#endif
...
static ssize_t i2c_xfer_size_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "CFG_CTS_MAX_I2C_XFER_SIZE: %d\n", CFG_CTS_MAX_I2C_XFER_SIZE);
}

static struct device_attribute driver_attr_i2c_xfer_size = {
		.attr = {
			.name = "i2c_xfer_size",
			.mode = S_IWUSR | S_IRUGO,
		},
		.show = i2c_xfer_size_show,
		.store = NULL,
};
...
late_initcall(cts_i2c_driver_init);
#else
