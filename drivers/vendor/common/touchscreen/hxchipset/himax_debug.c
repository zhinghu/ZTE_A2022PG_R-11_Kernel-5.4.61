...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd == 0) {
		...
		buf = kzalloc((hx_touch_data->touch_all_size * sizeof(uint8_t)), GFP_KERNEL);
		if (buf == NULL) {
			E("%s,alloc memory failed.\n", __func__);
			return;
		}
...
bool himax_ts_diag_func(void)
{
	...
	write_buf = kzalloc(total_size * 3, GFP_KERNEL);
	if (write_buf == NULL) {
		kfree(info_data);
		return false;
	}
	...
	if (dsram_type >= 1 && dsram_type <= 3) {
		...
		for (i = 0; i < x_channel * y_channel; i++) {
			...
			strlcat(&write_buf[i * strlen(temp_buf)], temp_buf, (total_size * 3 - strlen(write_buf)));
		}
		...
		for (i = 0; i < x_channel; i++) {
			...
			strlcat(&write_buf[(i + x_channel * y_channel) * strlen(temp_buf)], temp_buf,
				(total_size * 3 - strlen(write_buf)));
		}
