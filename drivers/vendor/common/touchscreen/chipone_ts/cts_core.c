			   ...
			   const u8 *src, size_t len, int retry, int delay)
{
	...
	while (len) {
		payload_len =
			min_t(size_t, (size_t)(max_xfer_size - cts_dev->rtdata.addr_width), len);
			  ...
			  u32 addr, void *dst, size_t len, int retry, int delay)
{
	...
	max_xfer_size = cts_plat_get_max_i2c_xfer_size(cts_dev->pdata);
	while (len) {
		xfer_len = min_t(size_t, max_xfer_size, len);
...
init_ic_type:
	...
	if ((flash_id == MD25D40_ID1) || (flash_id == MD25D40_ID2)
	    || (flash_id == MD25D20_ID1) || (flash_id == MD25D20_ID2)
	    || (flash_id == GD25Q10_ID) || (flash_id == MX25L512E_ID)
	    || (flash_id == MD25D05_ID) || (flash_id == MD25D10_ID)) {
...
{
	return cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_TX_ORDER, tx_order, TX_CHANNEL_NUM);
}

int cts_get_para_rx_order(struct cts_device *cts_dev, u8 *rx_order)
{
	return cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_RX_ORDER, rx_order, RX_CHANNEL_NUM);
}
			     ...
			     u32 addr, u8 n_col, void *buf, u8 is_diff)
{
	...
	cts_info("addr:%d, n_col:%d, fwdata.rows:%d,fwdata.cols:%d",
		addr, n_col, cts_dev->fwdata.rows, cts_dev->fwdata.cols);
...
{
	cts_info("Get raw data num_col=%d", cts_dev->hwdata->num_col);
...
int cts_wakeup_device(struct cts_device *cts_dev)
{
	...
	if (!cts_dev->hwdata->is_with_flash) {
		...
		if (strcmp(cts_dev->hwdata->name, "ICNT89xx") == 0) {
			...
			cts_dev->rtdata.addr_width   = 2;
			/* sram is broken */
			if (icnt87_sram_crc != icn87xx_calculate_crc(cts_dev, icnt87_sram_length)) {
...
err_set_program_mode:
	/* cts_dev->rtdata.program_mode = true; */
	/*  CTS_PROGRAM_MODE_I2CADDR; */
	/* cts_dev->rtdata.i2c_addr = cts_get_program_i2c_addr(cts_dev); */
...
u8 cts_get_program_i2c_addr(struct cts_device *cts_dev)
{
	...
	if (cts_plat_is_i2c_online(cts_dev->pdata, CTS_PROGRAM_MODE_I2CADDR)) {
		prog_i2c_addr =  CTS_PROGRAM_MODE_I2CADDR;
	} else if (cts_plat_is_i2c_online(cts_dev->pdata, CTS_PROGRAM_MODE_I2CADDR_2)) {
