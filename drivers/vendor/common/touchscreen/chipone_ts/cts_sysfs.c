...
static DEVICE_ATTR(write_reg, 0200, NULL, write_firmware_register_store);
...
static DEVICE_ATTR(read_reg, 0600,
...
static DEVICE_ATTR(curr_ver_info, 0444, curr_firmware_info_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (isdigit(*argv[0])) {
		ret = kstrtoul(argv[0], 0, &result);
		fw_index = (int)result;
		cts_info("firmware index: %d, ret=%d", fw_index, ret);
	} else {
...
static DEVICE_ATTR(driver_builtin_firmware, 0644,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (firmware) {
		...
		cts_info("update usage time = %4ldms",
...
static DEVICE_ATTR(update_from_file, 0200, NULL, update_firmware_from_file_store);
...
static DEVICE_ATTR(updating, 0444, updating_show, NULL);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Invalid version: %s\n", argv[0]);
	}
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "fw version test FAILED %d, version = 0x%x\n",
			       ret, version);
	} else {
		return snprintf(buf, PAGE_SIZE, "fw version test PASSED, version = 0x%x\n",
			       version);
	}
}
...
static DEVICE_ATTR(fw_version_test, 0644, fw_version_test_show, fw_version_test_store);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Invalid threshold_min: %s\n", argv[0]);
	}
	ret = kstrtou16(argv[1], 0, &th_max);
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Invalid threshold_max: %s\n", argv[1]);
	}
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE,
			"Rawdata test FAILED %d, threshold_min = %u threshold_max = %u\n",
			ret, th_min, th_max);
	} else {
		return snprintf(buf, PAGE_SIZE, "Rawdata test PASSED, threshold = %u threshold_max = %u\n",
			th_min, th_max);
	}
}
...
static DEVICE_ATTR(rawdata_test, 0644, rawdata_test_show, rawdata_test_store);
...
static DEVICE_ATTR(open_test, 0644, open_test_show, open_test_store);
...
static DEVICE_ATTR(short_test, 0644,
...
static DEVICE_ATTR(testing, 0444, testing_show, NULL);
...
static DEVICE_ATTR(cts_test_all, 0444, cts_test_all_show, NULL);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret == 0) {
		count = snprintf(buf, PAGE_SIZE - count, "reset pin test sucessful\n");
	} else {
		count = snprintf(buf, PAGE_SIZE - count, "reset pin test failed\n");
	}

	return count;
}
static DEVICE_ATTR(reset_test, 0444, reset_test_show, NULL);
#endif
...
static DEVICE_ATTR(ic_type, 0444, ic_type_show, NULL);
...
static DEVICE_ATTR(program_mode, 0644,
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "Normal mode: %s\n",
...
static DEVICE_ATTR(normal_mode, 0644,
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "is device enabled: %s\n",
...
static DEVICE_ATTR(device_en, 0644,
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "esd check enabled: %s\n",
...
static DEVICE_ATTR(esd_check, 0644,
...
#define RAWDATA_SELF_CAP_BUFFER_SIZE(cts_dev) \
	...
	self_cap = kmalloc(RAWDATA_SELF_CAP_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (self_cap == NULL) {
		kfree(rawdata);
		cts_err("Allocate memory for rawdata failed\n");
		return -ENOMEM;
	...
	if (ret) {
		count += snprintf(buf, PAGE_SIZE - count, "Enable read raw data failed %d\n", ret);
		goto err_free_rawdata;
	}

	ret = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
	if (ret) {
		count += snprintf(buf, PAGE_SIZE - count, "Send cmd QUIT_GESTURE_MONITOR failed %d\n", ret);
		goto err_free_rawdata;
	}
	msleep(50);
	...
	if (ret) {
		count += snprintf(buf, PAGE_SIZE - count, "Get raw data failed %d\n", ret);
		data_valid = false;
	...
	if (ret) {
		count += snprintf(buf, PAGE_SIZE - count, "Disable read raw data failed %d\n", ret);
...
#define DATA_FORMAT_STR     "%4u "
		...
		count += snprintf(buf + count, PAGE_SIZE - count, "\n\nself cap rawdata:\n");
		for (r = 0; r < (cts_dev->fwdata.rows + cts_dev->fwdata.cols)
			&& count < PAGE_SIZE; r++) {
...
static DEVICE_ATTR(diffdata, 0444, diffdata_show, NULL);
...
static DEVICE_ATTR(reset_pin, 0600, reset_pin_show, reset_pin_store);
#endif
...
static DEVICE_ATTR(irq_pin, 0444, irq_pin_show, NULL);
...
static DEVICE_ATTR(irq_info, 0444, irq_info_show, NULL);
