...
int cts_get_version_test_var(void)
{
	...
	if (i < 0) {
		...
		ret = sscanf(selftestdata->keyword[i].val, "0x%x",
...
int cts_get_rawdata_test_var(void)
{
	...
	if (i < 0) {
		...
		ret = sscanf(selftestdata->keyword[i].val, "%d", &selftestdata->rawdata_test_var.min_thres);
		if (ret < 0)
	...
	if (i < 0) {
		...
		ret = sscanf(selftestdata->keyword[i].val, "%d", &selftestdata->rawdata_test_var.max_thres);
		if (ret < 0)
...
int cts_get_open_test_var(void)
{
	...
	if (i < 0) {
		...
		ret = sscanf(selftestdata->keyword[i].val, "%d", &selftestdata->open_test_var.thres);
		if (ret < 0)
...
int cts_get_short_test_var(void)
{
	...
	if (i < 0) {
		...
		ret = sscanf(selftestdata->keyword[i].val, "%d", &selftestdata->short_test_var.thres);
		if (ret < 0)
...
int cts_parse_ini(void)
{
	...
	cts_info("%s inifilepath=%s", __func__, inifilepath);
	...
	fsize = file_inode(inifile)->i_size;
	...
	cts_info("%s fsize=%d pos=%d", __func__, fsize, pos);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pos = 0;
...
int cts_produce_test_info(void)
{
	...
	cts_print_test_info("OUTPUT RESULT LOG:,%s%s,\n", save_file_path, save_test_result_log_name);
