...
int cts_parse_volt_dt(struct cts_platform_data *pdata, struct device *dev)
{
	...
	if (IS_ERR(pdata->vdd_ana)) {
		cts_err("regulator get of vdd_ana failed");
		pdata->vdd_ana = NULL;
	}
...
int cts_volt_enable(struct cts_platform_data *pdata, int on)
{
	int ret;
	...
	if (on) {
		cts_info("power on");
		if (pdata->vdd_ana) {
			ret = regulator_enable(pdata->vdd_ana);
			if (ret) {
				cts_err("Regulator vdd_ana enable failed rc=%d", ret);
			}
		...
		cts_info("power off");
		if (pdata->vdd_ana) {
			ret = regulator_disable(pdata->vdd_ana);
			if (ret) {
				cts_err("Regulator vdd_ana disable failed rc=%d", ret);
			}
		}
	}
...
int cts_volt_remove(struct cts_platform_data *pdata)
{
	if (pdata == NULL)
		return -ENODEV;
...
int cts_plat_request_irq(struct cts_platform_data *pdata)
{
	...
	ret = request_irq(pdata->irq,
			  ...
			  pdata->i2c_client->dev.driver->name, pdata);
	cts_info("%s set irq as falling", __func__);
#endif /* CONFIG_GENERIC_HARDIRQS */
	if (ret) {
			       ...
			       struct cts_device_touch_msg *msgs, int num)
{
	...
	int i;
	int contact = 0;
	static unsigned char finger_down[CFG_CTS_MAX_TOUCH_NUM] = {0};
	...
	for (i = 0; i < num; i++) {
		...
		if (msgs[i].id >= CFG_CTS_MAX_TOUCH_NUM) {
			cts_err("touch id %d is invalid", msgs[i].id);
			continue;
		}
	...
	for (i = 0; i < CFG_CTS_MAX_TOUCH_NUM; i++) {
		if (finger_down[msgs[i].id] != 0 && finger_current[msgs[i].id] == 0) {
			input_mt_slot(input_dev, msgs[i].id);
			input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
