...
int  icn85xx_read_flashid(struct cts_device *cts_dev)
{
	...
	cts_info("flashid: 0x%06x", flashid);
...
static unsigned int icn85xx_crc_calc(unsigned int crc_in, char *buf, int len)
{
	int i;
...
int  icn87xx_read_flashid(struct cts_device *cts_dev)
{
	...
	cts_info("flashid: 0x%06x", flash_id);
...
static int icn87xx_erase_flash(struct cts_device *cts_dev, u8 u8EraseMode, u32 u32FlashAddr)
{
	...
	if (u8EraseMode == FLASH_EARSE_4K) {
		ucTemp[0] = FLASH_CMD_ERASE_SECTOR;
	} else if (u8EraseMode == FLASH_EARSE_32K) {
			       ...
			       u32 u32SramAddr, u32 u32Length)
{
	...
	for (i = 0; i < u32Length;) {                       /* should not i++ */
		...
		if (u32FlashTempAddr % 0x100) { /* not aglin */
			...
			/* change the flash and sram address */
			u32FlashTempAddr += u16NotAlignLength;
...
static int  icn87xx_fw_download(struct cts_device *cts_dev, unsigned int offset, unsigned char *buffer,
				unsigned int size)
{
#ifdef ENABLE_BYTE_CHECK
	char testb[B_SIZE];
			...
			const struct cts_firmware *firmware, bool to_flash)
{
	...
	/* contain sram length ,fwversion */
	ret = icn87xx_read_fw_info(firmware->data, temp_buf, FIRMWARA_INFO_AT_BIN_ADDR, 16);
...
unsigned int icn89xx_calculate_crc(struct cts_device *cts_dev, unsigned int len)
{
	...
	cts_info("icn89xx_calculate_crc: 0x%x", crc);
	return crc;
}
