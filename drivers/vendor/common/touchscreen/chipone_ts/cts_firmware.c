...
#if (CTS_CHIP_TYPE == ICNT89xx_TYPE)
	{
		"ICNT89xx", {
			{0x00, "ICNT89xx_00.bin",},
			{0x01, "ICNT89xx_01.bin",},
			{0x02, "ICNT89xx_02.bin",},
			{0x10, "ICNT89xx_10.bin",},
			{0x11, "ICNT89xx_11.bin",},
			{0x12, "ICNT89xx_12.bin",},
			{0x20, "ICNT89xx_20.bin",},
			{0x21, "ICNT89xx_21.bin",},
			{0x22, "ICNT89xx_22.bin",},
			{0xff, CTS_DEFAULT_FW_NAME,},
		},
	},
#elif (CTS_CHIP_TYPE == ICNT86xx_TYPE)
	{
		"ICNT86xx", {
			{0x00, CTS_FW_NAME0,},
			{0x01, CTS_FW_NAME1,},
...
#elif (CTS_CHIP_TYPE == ICNT87xx_TYPE)
	{
		"ICNT87xx", {
			{0x00, "ICNT87xx_00.bin",},
			{0x01, "ICNT87xx_01.bin",},
			{0x02, "ICNT87xx_02.bin",},
			{0x10, "ICNT87xx_10.bin",},
			{0x11, "ICNT87xx_11.bin",},
			{0x12, "ICNT87xx_12.bin",},
			{0x20, "ICNT87xx_20.bin",},
			{0x21, "ICNT87xx_21.bin",},
			{0x22, "ICNT87xx_22.bin",},
			{0xff, CTS_DEFAULT_FW_NAME,},
		},
	},
#elif (CTS_CHIP_TYPE == ICNT81xx_TYPE)
	{
		"ICNT81xx", {
			{0x00, "ICNT81xx_00.bin",},
			{0x01, "ICNT81xx_01.bin",},
			{0x02, "ICNT81xx_02.bin",},
			{0x10, "ICNT81xx_10.bin",},
			{0x11, "ICNT81xx_11.bin",},
			{0x12, "ICNT81xx_12.bin",},
			{0x20, "ICNT81xx_20.bin",},
			{0x21, "ICNT81xx_21.bin",},
			{0x22, "ICNT81xx_22.bin",},
			{0xff, CTS_DEFAULT_FW_NAME,},
		},
	},
#endif
...
static struct bin_in_fs  bin_table[] = {
	{ "ICNT89xx", "ICNT89xx.bin",},
	{ "ICNT86xx", "ICNT86xx.bin",},
	{ "ICNT87xx", "ICNT87xx.bin",},
	{ "ICNT85xx", "ICNT85xx.bin",},
	{ "ICNT82xx", "ICNT82xx.bin",},
	{ "ICNT81xx", "ICNT81xx.bin",},
	/* if you want support other chip  value ,please add here */
};
#endif
		...
		if ((strcmp(firmware->name, sensor_id_array_table[i].name) == 0)) {
			if (cts_dev->rtdata.is_chip_empty) {
				cts_info("Chip is empty return default firmware bultin: chip_index:%d id:0x%02x id_index: %d ",
					i, sensor_id_array_table[i].id_tables[MAX_SUPPORT_ID_NUM].id,
					MAX_SUPPORT_ID_NUM);
...
static int cts_dev_get_gpio_status(struct cts_device *cts_dev, int gpio, u8 *status)
{
	...
	cts_info("Get GPIO%u status", gpio);
...
#if (CTS_CHIP_TYPE == ICNT86xx_TYPE)
	ret = cts_dev_hw_reg_set_bit(cts_dev, CTS_DEVICE_HW_REG_PINMUX, gpio);
	if (ret) {
		cts_err("get gpio status,the reg set bit failed\n");
		return ret;
	}
#else
	ret = cts_dev_hw_reg_clr_bit(cts_dev, CTS_DEVICE_HW_REG_PINMUX, gpio);
	if (ret) {
		cts_err("get gpio status,the reg clr bit failed\n");
		return ret;
	}
#endif
...
int cts_dev_get_sensor_id(struct cts_device *cts_dev, u8 *sensor_id)
{
	...
	ret = cts_dev_get_gpio_status(cts_dev, CTS_SENSORID_GPIO1, &id1);
	if (ret) {
		cts_err("get gpio status id1 failed\n");
		return ret;
	}

	ret = cts_dev_get_gpio_status(cts_dev, CTS_SENSORID_GPIO2, &id2);
	if (ret) {
...
#ifdef SUPPORT_SENSOR_ID
			if (cts_dev->confdata.is_sensor_matched == 1)
#else
			if (1)
#endif
			{
	...
	const struct cts_device *cts_dev, const char *filepath, u16 curr_version)
{
	...
	if (ret) {
		cts_err("request firmware failed:%s ret=%d", filepath, ret);
		goto err_free_firmware;
	}
	firmware->size = fw->size;
	version = *((u16 *)(fw->data + firmware->ver_offset));
	cts_info("request file fw version:0x%04x, ver_offset=0x%x",
		version,  firmware->ver_offset);
...
const struct cts_firmware *cts_request_newer_firmware_from_sdcard(
	struct cts_device *cts_dev, const char *fwname, u16 curr_version)
{
	struct cts_firmware *firmware;
	struct file *file;
	int ret, read_size;
...
const struct cts_firmware *cts_request_firmware_from_sdcard(
	struct cts_device *cts_dev, const char *fwname)
{
	cts_info("Request from file '%s'", fwname);

	return cts_request_newer_firmware_from_sdcard(cts_dev, fwname, 0);
}
	...
	struct cts_device *cts_dev, u16 hwid, u16 fwid, u16 curr_firmware_ver)
{
	...
	if (ret) {
		...
		p = sensor_id_bin_table[cts_dev->confdata.fw_name_index]
			.id_tables[cts_dev->confdata.fw_sensor_id_index].bin;
		...
		firmware_from_file = cts_request_newer_firmware_from_fs(
			...
			? FIRMWARE_VERSION(firmware_builtin, firmware_builtin->ver_offset) :
			curr_firmware_ver);
	}
#else
