...
static int cts_resume(struct chipone_ts_data *cts_data)
{
	...
	cts_info("Resume");
	if (!cts_data->cts_dev.rtdata.suspended) {
		cts_warn("Resume device while already resume");
		return 0;
	}
				...
				const struct i2c_device_id *id)
{
	...
	cts_info("Driver for qualcomm platform %s", CFG_CTS_DRIVER_VERSION);
	...
	/* 2.8V voltage config */
	cts_parse_volt_dt(cts_data->pdata, &client->dev);
	cts_volt_enable(cts_data->pdata, 1);
	...
	cts_common_interface_register(&cts_data->cts_dev);
...
err_destroy_workqueue:
	...
	/* free 2.8V voltage */
	cts_volt_enable(cts_data->pdata, 0);
	cts_volt_remove(cts_data->pdata);
...
static DRIVER_ATTR(reset_pin, 0444, reset_pin_show, NULL);
...
static DRIVER_ATTR(swap_xy, 0444, swap_xy_show, NULL);
...
static DRIVER_ATTR(wrap_x, 0444, wrap_x_show, NULL);
...
static DRIVER_ATTR(wrap_y, 0444, wrap_y_show, NULL);
...
static DRIVER_ATTR(force_update, 0444, force_update_show, NULL);
...
static DRIVER_ATTR(max_touch_num, 0444, max_touch_num_show, NULL);
...
static DRIVER_ATTR(vkey, 0444, vkey_show, NULL);
...
static DRIVER_ATTR(gesture, 0444, gesture_show, NULL);
...
static DRIVER_ATTR(esd_protection, 0444, esd_protection_show, NULL);
...
static DRIVER_ATTR(slot_protocol, 0444, slot_protocol_show, NULL);
...
static DRIVER_ATTR(i2c_xfer_size, 0444, i2c_xfer_size_show, NULL);
