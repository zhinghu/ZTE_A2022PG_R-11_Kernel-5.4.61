...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (fbi[i].mode == mode && fbi[i].len != 0) {
			...
			if (ilitek_tddi_fw_iram_program(fbi[i].mem_start, (fw_ptr + fbi[i].start), fbi[i].len,
				SPI_UPGRADE_LEN) < 0)
			...
			if (mode != GESTURE) {
				ilitek_tddi_fw_iram_read(crc_temp,
					(fbi[i].mem_start + fbi[i].len - crc_len), sizeof(crc_temp));
			...
			ILI_INFO("%s CRC is %s hex(%x) : dma(%x) : iram(%x), calculation len is 0x%x\n",
				fbi[i].name, ((crc != dma) || (iram_crc_err)) ? "Invalid !" : "Correct !",
