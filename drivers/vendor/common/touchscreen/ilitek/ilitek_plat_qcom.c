...
#ifdef CONFIG_DRM_PANEL_NOTIFIER
static int ilitek_drm_register(void)
{
	struct device *dev = &idev->i2c->dev;
	int retval = -1;
	int i;
	int count;
	...
	for (i = 0; i < count; i++) {
		...
		if (PTR_ERR(panel) == -ENODEV) {
			...
			ipio_err("device has not been probed yet\n");
			retval = -EPROBE_DEFER;
		}
...
void ilitek_plat_irq_wake_disable(void)
{
	unsigned long flag;

	spin_lock_irqsave(&idev->irq_spin, flag);

	if (atomic_read(&idev->irq_wake_stat) == DISABLE)
		goto out;
	...
	ipio_debug("Disable irq wake success\n");

out:
	spin_unlock_irqrestore(&idev->irq_spin, flag);
}

void ilitek_plat_irq_wake_enable(void)
{
	unsigned long flag;

	spin_lock_irqsave(&idev->irq_spin, flag);

	if (atomic_read(&idev->irq_wake_stat) == ENABLE)
		goto out;
	...
	atomic_set(&idev->irq_wake_stat, ENABLE);
	ipio_debug("Enable irq success\n");
...
{
#if defined(CONFIG_FB) || defined(CONFIG_DRM_MSM) || defined(_NOTIFIER)
	ipio_info("Init notifier_fb struct\n");
	idev->notifier_fb.notifier_call = ilitek_plat_notifier_fb;
#ifdef CONFIG_DRM_PANEL_NOTIFIER
	if (ilitek_drm_register() == 0) {
		if (drm_panel_notifier_register(active_panel, &idev->notifier_fb) < 0)
			ipio_err("Failed to register FB notifier client\n");
	}
...
static int ilitek_plat_probe(void)
{
	...
	if (get_tp_chip_id() == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) && (tpd_fw_cdev.tp_chip_id != TS_CHIP_ILITEK	)) {
			ipio_info("this tp is not used,return.\n");
			return -EPERM;
		}
	}
#endif
