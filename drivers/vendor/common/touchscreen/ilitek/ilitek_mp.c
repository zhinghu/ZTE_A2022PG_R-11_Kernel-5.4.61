	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			...
			ipio_debug("(key: %s, value:%s) => (ini key: %s, val: %s)\n",
				   key, value, ini_info[i].key_name, ini_info[i].key_value);
			ret = 0;
			break;
		}
	}
...
void parser_ini_nodetype(s32 *type_ptr, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if ((strnstr(ini_info[i].section_name, desp, PARSER_MAX_KEY_NAME_LEN) <= 0) ||
					...
					if ((count % 4) == 3) {
						if (data[0] == 1) {
							if (type == VALUE) {
								...
								max_ptr[count / 4] =
								    data[1] + (data[1] * data[2]) / 100;
								min_ptr[count / 4] =
								    data[1] - (data[1] * data[3]) / 100;
							}
						} else {
...
/* char M_CFG_NIS = ':'; */
	...
	while (true) {
		...
		if (banchmark_flag) {
			...
			strlcpy(ini_info[g_ini_items].key_name, BENCHMARK_KEY_NAME,
		...
		ipio_debug("%s = %s\n", ini_info[g_ini_items].key_name, ini_info[g_ini_items].key_value);
...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
	...
	if (ERR_ALLOC_MEM(f)) {
		ipio_err("Failed to open ini file at %ld, trying to open md_ini_rq_path\n", PTR_ERR(f));
		f = NULL;
		f = filp_open(idev->md_ini_rq_path, O_RDONLY, 644);
		ipio_info("open path = %s\n", idev->md_ini_rq_path);
		if (ERR_ALLOC_MEM(f)) {
			ipio_err("open ini file failed\n");
			return -EINVAL;
		}
	}
...
static char *get_date_time_str(void)
{
	...
	snprintf(time_data_buf, sizeof(time_data_buf), "%04d%02d%02d-%02d%02d%02d",
		 ...
		 rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
	...
	tmp_line++;
	tmp_len +=
	    snprintf(csv + tmp_len, (file_size - tmp_len), "ILITek C-TP Utility V%s	%x : Driver Sensor Test\n",
	...
	tmp_line++;
	tmp_len +=
	    snprintf(csv + tmp_len, (file_size - tmp_len), "Panel information ,XCH=%d, YCH=%d\n", core_mp.xch_len,
		     core_mp.ych_len);
...
static void mp_print_csv_tail(char *csv, int *csv_len, int file_size)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].item_result == MP_DATA_PASS)
			tmp_len +=
			    snprintf(csv + tmp_len, (file_size - tmp_len), "	  {%s}	   ,OK\n", tItems[i].desp);
		else
			tmp_len +=
			    snprintf(csv + tmp_len, (file_size - tmp_len), "	  {%s}	   ,NG\n", tItems[i].desp);
	}
	...
	if (ipio_strcmp(name, "open test(integration)_sp") == 0) {
		...
		for (i = 0; i < size; i++) {
			...
			if (slen < 0)
				...
				tmp_len +=
				    snprintf(csv + tmp_len, (file_size - tmp_len), "%s = ,%s\n", open_sp_cmd[i], str);
		}
...
static s32 open_c_formula(int dac, int raw, int tvch, int gain)
{
	...
	if (type == ILI_R) {
		if (accuracy)
			ret =
			    ((dac * 376 * 39 / 2 * 10) +
			     ((raw - 8192) * 9 * (7 * 100 + 5) * 25 / 1024)) / tvch / 100 / gain;
		else
			ret =
			    ((dac * 376 * 39 / 2) +
			     ((raw - 8192) * 36 * (7 * 100 + 5) * 10 * 1 / 16384)) / tvch / 100 / gain;
	} else {
		if (accuracy)
			ret =
			    ((dac * 337 * 39 / 2 * 10) +
			     ((raw - 8192) * 9 * (7 * 100 - 22) * 25 / 1024)) / tvch / 100 / gain;
		else
			ret =
			    ((dac * 414 * 39 / 2) +
			     ((raw - 8192) * 36 * (7 * 100 - 22) * 10 * 1 / 16384)) / tvch / 100 / gain;
	}
...
static int codeToOhm(s32 Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	if (Code == 0) {
		...
		temp =
		    ((douTVCH - douTVCL) * douVariation * (douTDF1 - douTDF2) * (1 << 12) / (9 * Code * douCint)) * 100;
...
int check_int_level(bool high)
{
	...
	while (timer) {
		level = gpio_get_value(idev->tp_int);

		ipio_info("int GPIO[%d] level = %d\n", idev->tp_int, level);
		if (high) {
			if (level) {
...
static int open_test_sp(int index)
{
	...
	memset(open, 0, tItems[index].frame_count * sizeof(struct mp_test_P540_open));
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				if (full_open_rate_compare
				    (open[i].full_Open, open[i].cbk_700, x, y, tItems[index].node_type[addr],
		...
		ilitek_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10, core_mp.frame_len, core_mp.xch_len,
		...
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				tItems[index].buf[(i * core_mp.frame_len) + addr] =
				    compare_charge(open[i].charg_rate, x, y, tItems[index].node_type, Charge_AA,
						   Charge_Border, Charge_Notch);
				addr++;
			}
		}
...
static int open_test_cap(int index)
{
	...
	memset(open, 0, tItems[index].frame_count * sizeof(struct mp_test_open_c));
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if ((open[i].cap_dac == NULL) || (open[i].cap_raw == NULL) || (open[i].dcl_cap == NULL)) {
			ipio_err("Failed to allocate cap_dac/raw/cap  buffer\n");
			goto out;
		}
	}
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].catalog == PIN_TEST) {
			...
			csv_len +=
			    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "Test INT Pin = %d\n",
				     tItems[i].test_int_pin);
			pr_info("Pulse Test = %d\n", tItems[i].int_pulse_test);
			csv_len +=
			    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "Pulse Test = %d\n",
				     tItems[i].int_pulse_test);
			pr_info("Delay Time = %d\n", tItems[i].delay_time);
			csv_len +=
			    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "Delay Time = %d\n",
				     tItems[i].delay_time);
			continue;
		}
		...
		csv_len +=
		    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "Frame count = %d\n", tItems[i].frame_count);

		if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
			pr_info("lowest percentage = %d\n", tItems[i].lowest_percentage);
			csv_len +=
			    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "lowest percentage = %d\n",
				     tItems[i].lowest_percentage);

			pr_info("highest percentage = %d\n", tItems[i].highest_percentage);
			csv_len +=
			    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), "highest percentage = %d\n",
				     tItems[i].highest_percentage);
		}
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			for (y = 0; y < core_mp.key_len; y++) {
				...
				csv_len +=
				    snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len), " %3d, ", tItems[i].buf[y]);
			}
	...
	if (pass_item_count == 0) {
		...
		ret = MP_DATA_FAIL;
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s%s_%s.csv", idev->md_save_file_path,
