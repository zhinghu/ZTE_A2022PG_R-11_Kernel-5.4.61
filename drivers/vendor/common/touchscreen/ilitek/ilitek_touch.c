...
static void dma_clear_reg_setting(void)
{
	...
	if (ilitek_ice_mode_bit_mask_write
	    (DMA50_ADDR, DMA50_reg_dma_ch0_src1_format | DMA50_reg_dma_ch0_src1_en, BIT(31)) < 0)
		ipio_err("Write %lu at %x failed\n", DMA50_reg_dma_ch0_src1_format | DMA50_reg_dma_ch0_src1_en,
			 DMA50_ADDR);
...
static void dma_trigger_reg_setting(u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	...
	if (ilitek_ice_mode_write(DMA54_ADDR, DMA54_reg_dma_ch0_dest_format | DMA54_reg_dma_ch0_dest_en, BIT(31)) < 0)
		ipio_err("Write %lu at %x failed\n", DMA54_reg_dma_ch0_dest_format | DMA54_reg_dma_ch0_dest_en,
			 DMA54_ADDR);
	...
	if (ilitek_ice_mode_bit_mask_write
	    (INTR1_ADDR, INTR1_reg_dma_ch0_int_flag | INTR1_reg_flash_int_flag, BIT(17) | BIT(25)) < 0)
...
int ilitek_tddi_move_mp_code_flash(void)
{
	...
	if (overlay_start_addr != 0x0 && overlay_end_addr != 0x0 && cmd[0] == P5_X_MP_TEST_MODE_INFO)
...
#ifdef ILITEK_REPORT_BY_ZTE_ALGO
			if (MT_PRESSURE)
				tpd_touch_press(idev->input, touch_info[i].x, touch_info[i].y,
...
#else
				input_report_key(idev->input, BTN_TOUCH, 1);
				ilitek_tddi_touch_press(touch_info[i].x, touch_info[i].y, touch_info[i].pressure,
							touch_info[i].id);
				input_report_key(idev->input, BTN_TOOL_FINGER, 1);
#endif
				if (!finger_down[touch_info[i].id]) {
					finger_down[touch_info[i].id] = 1;
					ipio_info("touch down id: %d, coord [%d:%d]", touch_info[i].id,
...
#ifdef ILITEK_REPORT_BY_ZTE_ALGO
					tpd_touch_release(idev->input, i);
#else
					ilitek_tddi_touch_release(0, 0, i);
#endif
					if (finger_down[i]) {
						finger_down[i] = 0;
						ipio_info("touch up id: %d, coord [%d:%d]", i, point_log[i].x, point_log[i].y);
					}
				}
