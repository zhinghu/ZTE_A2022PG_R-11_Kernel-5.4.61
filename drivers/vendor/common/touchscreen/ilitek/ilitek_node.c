...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (type == read) {
		...
		len +=
		    snprintf(g_user_buf + len, USER_STR_BUFF - len, "READ:addr = 0x%06x, read = 0x%08x\n", addr,
			     read_data);
	} else {
		...
		len +=
		    snprintf(g_user_buf + len, USER_STR_BUFF - len,
...
static ssize_t ilitek_proc_debug_switch_read(struct file *pFile, char __user *buff, size_t size, loff_t *pos)
{
	...
	size =
	    snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "dnp : %s\n", idev->dnp ? "Enable" : "Disable");
	*pos = size;
...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (idev->dbl[idev->odi].mark) {
		...
		for (i = 0; i < need_read_data_len; i++) {
			send_data_len +=
			    snprintf(tmpbuf + send_data_len, sizeof(tmpbufback), "%02X", idev->dbl[idev->odi].data[i]);
...
static ssize_t ilitek_proc_get_debug_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	csv.wlen +=
	    snprintf(csv.ptr + csv.wlen, (csv.max_size - csv.wlen), "Get Delta data %d frame\n", idev->delta_count);
...
static ssize_t ilitek_node_mp_lcm_on_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (g_user_buf[1] == EMP_MODE) {
		...
		len +=
		    snprintf(g_user_buf + len, USER_STR_BUFF - len, "%s\n", "Protocol version isn't matched, abort!");
...
static ssize_t ilitek_node_mp_lcm_off_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (ret != 0)
		ipio_err("Failed to create directory for mp_test\n");
	idev->md_save_file_path = CSV_LCM_OFF_PATH;
	if (esd_en)
		ilitek_tddi_wq_ctrl(WQ_ESD, DISABLE);
	if (bat_en)
		ilitek_tddi_wq_ctrl(WQ_BAT, DISABLE);
...
static ssize_t ilitek_proc_debug_level_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	size =
	    snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "debug level : %s\n",
...
static long ilitek_node_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_COMPAT_IOCTL_I2C_SET_WRITE_LENGTH:
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_I2C_SET_WRITE_LENGTH, (unsigned long)compat_ptr(arg));
		return ret;
	case ILITEK_COMPAT_IOCTL_I2C_SET_READ_LENGTH:
		ipio_debug("compat_ioctl: convert set read length\n");
		ret =
		    filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_I2C_SET_READ_LENGTH, (unsigned long)compat_ptr(arg));
		return ret;
	...
	case ILITEK_COMPAT_IOCTL_TP_PANEL_INFO:
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, ILITEK_COMPAT_IOCTL_TP_PANEL_INFO, (unsigned long)compat_ptr(arg));
		return ret;
