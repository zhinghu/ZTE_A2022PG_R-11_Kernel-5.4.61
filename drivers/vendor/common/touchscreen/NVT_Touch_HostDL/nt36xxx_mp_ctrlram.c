...
extern int novatek_tptest_result;
...
static void nvt_print_data_log_in_one_line(int32_t *data, int32_t data_num)
{
	...
	for (i = 0; i < data_num; i++) {
		snprintf(tmp_log + i * 7, ((data_num * 7 + 1) - i * 7), "%5d, ", data[i]);
	}
...
static void nvt_print_result_log_in_one_line(uint8_t *result, int32_t result_num)
{
	...
	for (i = 0; i < result_num; i++) {
		snprintf(tmp_log + i * 6, ((result_num * 6 + 1) - i * 6), "0x%02X, ", result[i]);
	}
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path,
				       uint32_t offset)
{
	int32_t x = 0;
	int32_t y = 0;
	...
	for (y = 0; y < y_ch; y++) {
		for (x = 0; x < x_ch; x++) {
			...
			len += snprintf(fbufp + iArrayIndex * 7 + y * 2, 8192 - len, "%5d, ", rawdata[iArrayIndex]);
		}
		nvt_print_data_log_in_one_line(rawdata + y * x_ch, x_ch);
		NVT_LOG("\n");
		len += snprintf(fbufp + (iArrayIndex + 1) * 7 + y * 2,  8192 - len, "\r\n");
	}
#if TOUCH_KEY_NUM > 0
	keydata_output_offset = y_ch * x_ch * 7 + y_ch * 2;
	for (k = 0; k < Key_Channel; k++) {
		iArrayIndex = y_ch * x_ch + k;
		len += snprintf(fbufp + keydata_output_offset + k * 7,  8192 - len, "%5d, ", rawdata[iArrayIndex]);
	}
	nvt_print_data_log_in_one_line(rawdata + y_ch * x_ch, Key_Channel);
	NVT_LOG("\n");
	len += snprintf(fbufp + y_ch * x_ch * 7 + y_ch * 2 + Key_Channel * 7,  8192 - len, "\r\n");
#endif /* #if TOUCH_KEY_NUM > 0 */
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	if (nvt_save_rawdata_to_csv
	    (RawData_Diff_Min, X_Channel, Y_Channel, NOISE_TEST_CSV_FILE, rawdata_diff_min_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		for (x = 0; x < X_Channel; x++) {
			...
			xdata[iArrayIndex] =
			    (int16_t) (rawdata_buf[iArrayIndex * 2] + 256 * rawdata_buf[iArrayIndex * 2 + 1]);
		}
	}
#if TOUCH_KEY_NUM > 0
	for (k = 0; k < Key_Channel; k++) {
...
*******************************************************/
static int32_t RawDataTest_SinglePoint_Sub(int32_t rawdata[], uint8_t RecordResult[], uint8_t x_ch, uint8_t y_ch,
...
return:
	n.a.
*******************************************************/
void print_selftest_result(struct seq_file *m, int32_t TestResult, uint8_t RecordResult[], int32_t rawdata[],
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		print_selftest_result(m, TestResult_FWMutual, RecordResult_FWMutual, RawData_FWMutual, X_Channel,
				      Y_Channel);
	} else {		/* TestResult_FW_Rawdata is -1 */
		nvt_mp_seq_printf(m, " FAIL!\n");
		if (TestResult_FWMutual == -1) {
			nvt_mp_seq_printf(m, "FW Mutual");
			print_selftest_result(m, TestResult_FWMutual, RecordResult_FWMutual, RawData_FWMutual,
					      X_Channel, Y_Channel);
		}
		if (TestResult_FW_CC == -1) {
			nvt_mp_seq_printf(m, "FW CC");
			print_selftest_result(m, TestResult_FW_CC, RecordResult_FW_CC, RawData_FW_CC, X_Channel,
					      Y_Channel);
		}
	}
	...
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		print_selftest_result(m, TestResult_FW_DiffMax, RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel,
				      Y_Channel);
		...
		if (TestResult_FW_DiffMin == -1) {
			...
			print_selftest_result(m, TestResult_FW_DiffMin, RecordResult_FW_DiffMin, RawData_Diff_Min,
					      X_Channel, Y_Channel);
		}
	}
...
int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_read_baseline(RawData_FWMutual) != 0) {
		...
		TestResult_FWMutual =
		    RawDataTest_SinglePoint_Sub(RawData_FWMutual, RecordResult_FWMutual, X_Channel, Y_Channel,
	...
	if (nvt_read_fw_noise(RawData_Diff) != 0) {
		...
		TestResult_FW_DiffMax =
		    RawDataTest_SinglePoint_Sub(RawData_Diff_Max, RecordResult_FW_DiffMax, X_Channel, Y_Channel,
						PS_Config_Lmt_FW_Diff_P, PS_Config_Lmt_FW_Diff_N);

		TestResult_FW_DiffMin =
		    RawDataTest_SinglePoint_Sub(RawData_Diff_Min, RecordResult_FW_DiffMin, X_Channel, Y_Channel,
						PS_Config_Lmt_FW_Diff_P, PS_Config_Lmt_FW_Diff_N);
	...
	NVT_LOG("TestResult_FW_Rawdata:%d, TestResult_Noise:%d TestResult_Short:%dTestResult_Open:%d--\n",
	TestResult_FW_Rawdata, TestResult_Noise, TestResult_Short, TestResult_Open);
	if (TestResult_FW_Rawdata || TestResult_Noise)
		novatek_tptest_result = novatek_tptest_result | TEST_BEYOND_MAX_LIMIT | TEST_BEYOND_MIN_LIMIT;
	if (TestResult_Short)
		novatek_tptest_result = novatek_tptest_result | TEST_GT_SHORT;
	if (TestResult_Open)
		novatek_tptest_result = novatek_tptest_result | TEST_GT_OPEN;
	return 0;
}
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_P", PS_Config_Lmt_FW_CC_P, X_Channel * Y_Channel + Key_Channel))
		return -EPERM;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_N", PS_Config_Lmt_FW_CC_N, X_Channel * Y_Channel + Key_Channel))
		return -EPERM;
