...
{
	FTS_INFO("%s wakeup_gesture_enable val is:%d.", __func__, fts_gesture_data.mode);
...
int tpd_set_gesture_mode(int gesture_mode)
{
	...
	FTS_INFO("%s previous val is:%d, current val is:%d.", __func__, fts_gesture_data.mode,
		 gesture_mode);
...
int fts_gesture_resume(struct i2c_client *client)
{
	...
	if (fts_gesture_data.active == DISABLE) {
		FTS_DEBUG("gesture in suspend is failed, no running %s", __func__);
		return -EINVAL;
	}
