...
static int fts_test_get_save_filename(char *filename, char *save_filename, int len)
{
	...
	if (IS_ERR(pfile)) {
		...
		if (pValid) {
			snprintf(save_filename, len, "TP_test_data%04d%02d%02d%02d%02d%02d.csv", tm.tm_year + 1900,
				 tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		} else {
			pValid = strnstr(filename, ".txt", 50);
			snprintf(save_filename, len, "testresult%04d%02d%02d%02d%02d%02d.txt", tm.tm_year + 1900,
				 tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		}
		return -EIO;
	}
...
static int tpd_test_cmd_show(struct tpd_classdev_t *cdev, char *buf)
{
	...
	i_len = snprintf(buf, PAGE_SIZE, "%d,%d,%d,%d", g_int_tptest_result,
			 test_data.screen_param.tx_num, test_data.screen_param.rx_num,
			 test_data.rawdata_failed_count);
	FTS_TEST_INFO("tpd test result:0x%x && rawdata node failed count:0x%x.\n", g_int_tptest_result,
		      test_data.rawdata_failed_count);
	FTS_TEST_INFO("tpd result && failed cout string length:0x%x.\n", i_len);
