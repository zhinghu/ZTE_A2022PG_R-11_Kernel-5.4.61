		...
		if (ReCode == ERROR_CODE_OK) {
			while (times++ < MaxTimes) {	/* Wait for the scan to complete */
				...
				if (ReCode == ERROR_CODE_OK) {
					...
					FTS_TEST_ERROR("%s read DEVIDE_MODE_ADDR error.", __func__);
					break;
				}
			}
			if (times < MaxTimes)
			...
			FTS_TEST_ERROR("%s write DEVIDE_MODE_ADDR error.", __func__);
	} else
		FTS_TEST_ERROR("%s read DEVIDE_MODE_ADDR error.", __func__);
	return ReCode;
