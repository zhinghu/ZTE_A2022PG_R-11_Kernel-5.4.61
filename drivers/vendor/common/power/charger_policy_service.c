...
#define POWER_SUPPLY_PROP_CHARGING_ENABLED		"/sys/class/qcom-battery/charging_enabled"
#define POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED	"/sys/class/qcom-battery/battery_charging_enabled"
#define POWER_SUPPLY_PROP_USB_SUSPEND			"/sys/class/qcom-battery/usb_suspend"
#define POWER_SUPPLY_PROP_USB_PRESENT			"/sys/class/qcom-battery/usb_present"
...
static int charger_policy_set_batt_prop_by_name(char *path, char *buf)
{
	...
	if (IS_ERR(filp)) {
		pr_policy("unable to open file: %s\n", path);
		return -1;
	}
...
static int charger_policy_get_batt_prop_by_name(char *path, bool *result)
{
	struct file *filp = NULL;
	loff_t pos = 0;
	char   buf[10];
...
{
	int rc = 0;
	bool battery_charging_enabled = false, charging_enabled = false;
	...
	/*get info*/
	rc = charger_policy_get_batt_prop_by_name(POWER_SUPPLY_PROP_CHARGING_ENABLED, &charging_enabled);
	if (rc < 0) {
	...
	rc = charger_policy_get_batt_prop_by_name(POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED, &battery_charging_enabled);
	if (rc < 0) {
	...
	rc = charger_policy_set_batt_prop_by_name(POWER_SUPPLY_PROP_CHARGING_ENABLED, buf);
	if (rc < 0) {
	...
	rc = charger_policy_set_batt_prop_by_name(POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED, buf);
	if (rc < 0) {
