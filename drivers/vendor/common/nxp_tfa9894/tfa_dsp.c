...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	enum tfa_error tfa_err = tfa_error_ok;
	...
	pr_info("tfa_dev(%d): Active_profile:%s, next_profile:%s\n",
			tfa->dev_idx,
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	if ((tfa->tfa_family == 2) && (tfa->is_probus_device == 0)) {
		...
		pr_info("tfa_dev(%d): exit: MANSTATE %d, tries %d\n", tfa->dev_idx, manstate, tries);
	}
...
{
	enum Tfa98xx_Error tfa98xx_err = Tfa98xx_Error_Ok;
	int loop = 50, ready = 0;
	int count;
	...
	if(!tfa->is_probus_device)
		...
		do {
			tfa98xx_err = tfa98xx_dsp_system_stable(tfa, &ready);
			if (tfa98xx_err != Tfa98xx_Error_Ok)
				return tfa_error_device;
			if (ready)
				break;
		} while (loop--);
 if (((!tfa->is_probus_device) && (is_calibration)) || ((tfa->rev & 0xff) == 0x13))
	{
		...
		tfa98xx_err = tfa98xx_faim_protect(tfa, 1);
		if (tfa->verbose) {
			pr_debug("FAIM enabled (err:%d).\n", tfa98xx_err);
		}
	}
 ...
 if (((!tfa->is_probus_device) && (is_calibration)) || ((tfa->rev & 0xff) == 0x13))
	{ 
		tfa98xx_err = tfa98xx_faim_protect(tfa, 0);
		if (tfa->verbose) {
			pr_debug("FAIM disabled (err:%d).\n", tfa98xx_err);
		}
	}
...
enum tfa_error tfa_dev_mtp_set(struct tfa_device *tfa, enum tfa_mtp item, int value)
{
	...
	switch (item) {
		case TFA_MTP_OTC:
			tfa98xx_err = tfa98xx_set_mtp(tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK);
			break;
		case TFA_MTP_EX:
			tfa98xx_err = tfa98xx_set_mtp(tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK);
			break;
		case TFA_MTP_RE25:
		case TFA_MTP_RE25_PRIM:
			if(tfa->tfa_family == 2) {
