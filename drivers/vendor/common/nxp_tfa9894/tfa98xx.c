...
/* ZTE_chenjun: Start */
#define TFA_DEFAULT_PROFILE 0
#define DSP_STABLE_DELAY_MS 10
#define DSP_STABLE_DELAY_CNT 30
/* ZTE_chenjun: End */
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("enter new_profile [%d] profile [%d]\n", profile, ucontrol->value.integer.value[0]);
	...
	if (new_profile == profile) {
		pr_info("new_profile equal profile (%d)\n", new_profile);
		return 0;
	}
...
/* ZTE_chenjun: Start */
		if (ready && (tfa98xx->dsp_init == TFA98XX_DSP_INIT_STOPPED)) {
			ready = 0;
			pr_info("%s: tfa_dev(%d) dsp_init(%d) change ready(%d)\n",
					__func__, tfa98xx->tfa->dev_idx, tfa98xx->dsp_init, ready);
		} else if (ready && (new_profile == TFA_DEFAULT_PROFILE)) {
			pr_info("%s: tfa_dev(%d) check stable ready\n",
					__func__, tfa98xx->tfa->dev_idx);

			/* now wait for the DSP stable */
			do {
				msleep_interruptible(DSP_STABLE_DELAY_MS);
				tfa98xx_dsp_system_stable(tfa98xx->tfa, &ready);
				tries++;
			}  while ((tries < DSP_STABLE_DELAY_CNT) && ready);

			pr_info("%s: tfa_dev(%d) stable ready(%d), tries(%d)\n",
					__func__, tfa98xx->tfa->dev_idx, ready, tries);
		}
/* ZTE_chenjun: End */
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->reset_gpio)) {
		...
		if (ret) {
			dev_err(&i2c->dev, "Failed to request reset_gpio: %d\n",
					ret);
			return ret;
		}
	}
	...
	if (gpio_is_valid(tfa98xx->irq_gpio)) {
		...
		if (ret) {
			dev_err(&i2c->dev, "Failed to request irq_gpio: %d\n",
					ret);
			return ret;
		}
	}
