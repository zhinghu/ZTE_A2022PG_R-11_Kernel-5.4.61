...
u64 sdlog_adsp_mem_phys_addr = 0;
u32 sdlog_adsp_mem_size = 0;
...
static int is_f3mem_range(unsigned long pfn, unsigned long size)
{
	...
	if ((from >= sdlog_mem_phys_addr) && (to <= (sdlog_mem_phys_addr + sdlog_mem_size))) {
		pr_info("is_f3mem_range: belong to sdlog f3mem\n");
		return 1;
	} else if ((from >= sdlog_adsp_mem_phys_addr) &&
		(to <= (sdlog_adsp_mem_phys_addr + sdlog_adsp_mem_size))) {
		pr_info("is_f3mem_range: belong to adsp f3mem\n");
		return 1;
	}
	return 0;
}
...
static int __init msm_tb_dev_init(void)
{
	...
	if (!np) {
		...
		sdlog_adsp_mem_phys_addr = sdlog_adsp_memory_get_addr();
		sdlog_adsp_mem_size = sdlog_adsp_memory_get_size();
